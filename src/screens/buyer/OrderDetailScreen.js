...
// What button to show based on current status
const NEXT_ACTION = {
  ...
  preparing:  { label: 'Mark as Dispatched', next: 'dispatched', color: '#8B5CF6', icon: 'bicycle' },
...
export default function VendorOrderDetailScreen({ navigation, route }) {
  ...
  const handleUpdateStatus = async (newStatus) => {
    Alert.alert(
      'Confirm',
      `Mark order as "${STATUS_INFO[newStatus]?.label}"?`,
      [
  ...
  const handleReject = async () => {
    setUpdating(true);
    try {
      await client.post(`/orders/${orderId}/status/`, {
        status: 'rejected',
        reason: rejectReason || 'Rejected by vendor',
      });
      setShowReject(false);
      await fetchOrder();
    } catch (e) {
  ...
  const handleCallBuyer = () => {
    if (order?.buyer_phone) {
      Linking.openURL(`tel:${order.buyer_phone}`)
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 160 }}>
        ...
        <View style={[styles.statusBanner, { backgroundColor: statusInfo.bg, borderColor: statusInfo.color + '40' }]}>
          ...
          <View style={styles.statusInfo}>
            ...
            <Text style={styles.statusSub}>
              {isNew ? 'Respond within 10 minutes' :
               isDelivered ? 'Order completed successfully' :
        ...
        {/* ── Progress Status Update (non-new, non-terminal orders) ── */}
        {!isNew && !isCancelled && !isDelivered && nextAction && (
          <View style={styles.progressCard}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleUpdateStatus(nextAction.next)}
              disabled={updating}
            >
          ...
          <View style={[styles.progressCard, { backgroundColor: '#F0FDF4' }]}>
            <View style={styles.deliveredRow}>
              ...
              <View>
                ...
                <Text style={styles.progressSub}>Payment collected: ₹{total.toFixed(0)}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.addressBtns}>
            ...
            <TouchableOpacity style={styles.callBtnSmall} onPress={handleCallBuyer}>
              ...
              <Text style={styles.callBtnSmallText}>Call Buyer</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          {order.notes && (
            <>
              <View style={styles.divider} />
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Note from buyer</Text>
                <Text style={[styles.infoValue, { color: '#EA580C', flex: 1, textAlign: 'right' }]}>{order.notes}</Text>
              </View>
            </>
          )}
        </View>
        ...
        <View style={styles.modalCard}>
          ...
          <View style={styles.reasonBtns}>
            {['Item out of stock', 'Shop is closed', 'Cannot deliver to this area', 'Other'].map(reason => (
              ...
              >
                <Text style={[styles.reasonBtnText, rejectReason === reason && styles.reasonBtnTextActive]}>
                  {reason}
                </Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  rejectBtn: {
    ...
    gap: 6, borderWidth: 1.5, borderColor: '#DC2626',
  ...
  mapsBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 6, backgroundColor: '#eff6ff', borderRadius: 10, padding: 10,
  },
  mapsBtnText:    { fontSize: 13, color: '#1669ef', fontWeight: '600' },
  callBtnSmall: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 6, backgroundColor: '#F0FDF4', borderRadius: 10, padding: 10,
  },
