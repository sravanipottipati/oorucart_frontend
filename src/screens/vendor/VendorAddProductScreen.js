...
export default function VendorAddProductScreen({ navigation, route }) {
  ...
  const addVariant = () => {
    setVariants([...variants, { name: '', mrp: '', price: '', available: true }]);
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (image) {
        ...
        productRes = await client.post('/vendors/products/add/', productData, {
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.formCard}>
          ...
          {parseFloat(gst) > 0 && (
            <View style={styles.gstInfo}>
              ...
              <Text style={styles.gstInfoText}>
                GST: ₹{((parseFloat(price) || 0) * parseFloat(gst) / 100).toFixed(2)} · Total: ₹{((parseFloat(price) || 0) * (1 + parseFloat(gst) / 100)).toFixed(2)}
              </Text>
            </View>
          )}
          ...
          {/* ── Variants ── */}
          <Text style={styles.fieldLabel}>
            Variants <Text style={styles.optional}>(Optional — e.g. 500g, 1kg)</Text>
          </Text>
          ...
          {variants.length > 0 && (
            <View style={styles.variantTable}>
              ...
              <View style={styles.variantHeaderRow}>
                ...
                <Text style={[styles.variantHeaderCell, { flex: 1.5 }]}>Price (₹)</Text>
              ...
              {variants.map((v, index) => (
                <View key={index} style={[styles.variantRow, index % 2 === 1 && { backgroundColor: '#F9FAFB' }]}>
                  ...
                  />
                  <TextInput
                    style={[styles.variantInput, { flex: 1.5 }]}
                    placeholder="0"
                    placeholderTextColor="#9CA3AF"
                    value={v.mrp}
                    onChangeText={val => updateVariant(index, 'mrp', val)}
                    keyboardType="numeric"
                  />
                  <TextInput
                    style={[styles.variantInput, { flex: 1.5 }]}
                    ...
                    keyboardType="numeric"
                  />
                  <View style={[styles.variantStatusCell, { flex: 1.5 }]}>
                    <Switch
                      value={v.available}
                      onValueChange={val => updateVariant(index, 'available', val)}
                      trackColor={{ false: '#E5E7EB', true: '#86efac' }}
                      thumbColor={v.available ? '#16A34A' : '#9CA3AF'}
                      style={{ transform: [{ scaleX: 0.8 }, { scaleY: 0.8 }] }}
                    />
                  </View>
                  <TouchableOpacity
                    style={[styles.variantDeleteBtn, { flex: 0.6 }]}
...
const styles = StyleSheet.create({
  ...
  // Image picker
  imagePicker:          { borderWidth: 1.5, borderColor: '#E5E7EB', borderRadius: 12,  overflow: 'hidden' },
  imagePlaceholder:     { height: 140, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F9FAFB', gap: 8 },
  imageIconBox:         { width: 64, height: 64, borderRadius: 32, backgroundColor: '#eff6ff', justifyContent: 'center', alignItems: 'center', marginBottom: 4 },
  ...
  categoryChip:           { paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20, backgroundColor: '#F3F4F6', borderWidth: 1, borderColor: '#E5E7EB' },
  ...
  variantHeaderCell: { fontSize: 12, fontWeight: '700', color: '#555', textAlign: 'center' },
  ...
  variantStatusCell: { alignItems: 'center', justifyContent: 'center' },
  variantDeleteBtn:  { alignItems: 'center', justifyContent: 'center' },

  // Add Variant Button
  addVariantBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 8, borderWidth: 1.5, borderColor: '#1669ef',
